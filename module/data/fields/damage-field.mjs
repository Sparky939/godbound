...
export class DamageData extends foundry.abstract.DataModel {
    ...
    _automaticFormula(increase = 0) {
        ...
        if (number && this.denomination)
            formula = `${number}d${this.denomination}`
        if (this.bonus)
            formula = formula ? `${formula} + ${this.bonus}` : this.bonus
        return formula ?? ''
    }
    ...
    scaledFormula(increase) {
        ...
        if (this.custom.enabled) {
            ...
            formula = formula.replace(
                /^(\d)+d/,
                (match, number) => `${Number(number) + dieIncrease}d`
            )
        } else {
    ...
    steppedDenomination(steps = 1) {
        return (
            GODBOUND.dieSteps[
                Math.min(
                    GODBOUND.dieSteps.indexOf(this.denomination) + steps,
                    GODBOUND.dieSteps.length - 1
                )
            ] ?? null
        )
    }
