...
export default class BaseActivityData extends foundry.abstract.DataModel {
    ...
    get canConfigureScaling() {
        return (
            this.consumption.scaling.allowed ||
            (this.isSpell && this.item.system.level > 0)
        )
    }
    ...
    get canScale() {
        return (
            this.consumption.scaling.allowed ||
            (this.isSpell &&
                this.item.system.level > 0 &&
                GODBOUND.spellPreparationModes[
    ...
    static createInitialActivity(source, { offset = 0, ...options } = {}) {
        const activityData = this.transformTypeData(
            source,
            {
        ...
        foundry.utils.setProperty(
            source,
            `system.activities.${activityData._id}`,
            activityData
        )
        foundry.utils.setProperty(
            source,
    ...
    static transformActivationData(source, options) {
        ...
        return {
            type:
                source.system.activation?.type === 'none'
                    ? ''
    ...
    static transformConsumptionData(source, options) {
        ...
        else if (source.system.uses?.max)
            targets.push({
                type: 'itemUses',
                target: '',
                value: '1',
        ...
        if (source.system.recharge?.value)
            targets.push({
    ...
    static transformDamagePartData(source, [formula, type]) {
        ...
        const parsed = (formula ?? '').match(
            /^\s*(\d+)d(\d+)(?:\s*([+|-])\s*(@?[\w\d.-]+))?\s*$/i
        )
        if (parsed && GODBOUND.dieSteps.includes(Number(parsed[2]))) {
            data.number = Number(parsed[1])
            data.denomination = Number(parsed[2])
            if (parsed[4])
                data.bonus = parsed[3] === '-' ? `-${parsed[4]}` : parsed[4]
        } else if (formula) {
        ...
        if (
            (scaling && Number(scaling[2]) === data.denomination) ||
            source.system.scaling?.mode === 'cantrip'
        ) {
    ...
    static transformDurationData(source, options) {
        if (source.type === 'spell') return {}
        const concentration = !!source.system.properties?.findSplice(
            (p) => p === 'concentration'
        )
        return {
            concentration,
        ...
        return source.effects
            .filter(
                (e) =>
                    !e.transfer &&
                    e.type !== 'enchantment' &&
                    e.flags?.dnd5e?.type !== 'enchantment'
            )
            .map((e) => ({ _id: e._id }))
    }
    ...
    static transformTargetData(source, options) {
        ...
        if (source.system.target?.type in GODBOUND.areaTargetTypes)
    ...
    static transformUsesData(source, options) {
        if (!source.system.recharge?.value || !source.system.uses?.max)
        ...
        for (const target of this.consumption.targets) {
            ...
            if (!actor.items.get(target.target)) {
                const message = game.i18n.format(
                    'DND5E.CONSUMPTION.Warning.MissingItem',
                    {
                        activity: this.name,
                        item: this.item.name,
                    }
                )
                actor._preparationWarnings.push({
                    message,
                    link: this.uuid,
                    type: 'warning',
                })
    ...
    prepareDamageLabel(parts, rollData) {
        this.labels.damage = parts.map((part) => {
            ...
            try {
                ...
                if (part.base) {
                    if (this.item.system.magicAvailable)
                        formula += ` + ${this.item.system.magicalBonus ?? 0}`
                    if (this.item.type === 'weapon' && !/@mod\b/.test(formula))
                        formula += ' + @mod'
                }
                ...
                console.warn(
                    `Unable to simplify formula for ${this.name} in item ${this.item.name}${
                        this.actor
                            ? ` on ${this.actor.name} (${this.actor.id})`
                            : ''
                    } (${this.uuid})`,
                    err
                )
            ...
            let label = formula
            if (part.types.size) {
                label = `${formula} ${game.i18n
                    .getListFormatter({ type: 'conjunction' })
                    .format(
                        Array.from(part.types)
                            .map(
                                (p) =>
                                    GODBOUND.damageTypes[p]?.label ??
                                    GODBOUND.healingTypes[p]?.label
                            )
                            .filter((t) => t)
                    )}`
            }
            ...
            return {
                formula,
                damageType: part.types.size === 1 ? part.types.first() : null,
                label,
                base: part.base,
            }
        })
    ...
    getDamageConfig(config = {}) {
        if (!this.damage?.parts)
            return foundry.utils.mergeObject({ rolls: [] }, config)

        const rollConfig = foundry.utils.mergeObject({ scaling: 0 }, config)
        const rollData = this.getRollData()
        rollConfig.rolls = this.damage.parts
            .map((d, index) =>
                this._processDamagePart(d, rollConfig, rollData, index)
            )
    ...
    _processDamagePart(damage, rollConfig, rollData, index = 0) {
        ...
        const bonus = foundry.utils.getProperty(
            this.actor ?? {},
            `system.bonuses.${this.actionType}.damage`
        )
        ...
        return {
            ...
            options: {
                type:
                    this.item.getFlag(
                        'dnd5e',
                        `last.${this.id}.damageType.${index}`
                    ) ?? damage.types.first(),
                types: Array.from(damage.types),
                properties: Array.from(
                    this.item.system.properties ?? []
                ).filter((p) => GODBOUND.itemProperties[p]?.isPhysical),
            },
        }
    }
    ...
    _setOverride(keyPath) {
        ...
        if (obj.canOverride && !obj.override) {
            foundry.utils.mergeObject(
                obj,
                foundry.utils.getProperty(this.item.system, keyPath)
            )
        }
    }
}
