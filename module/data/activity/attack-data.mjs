...
export default class AttackActivityData extends BaseActivityData {
    ...
    static defineSchema() {
        return {
            ...
            damageParts: new ArrayField(new DamageField()),
        }
    }
    ...
    get ability() {
        ...
        if (this.attack.ability in GODBOUND.abilities)
        ...
        return availableAbilities.reduce(
            (largest, ability) =>
                (abilities[ability]?.mod ?? -Infinity) >
                (abilities[largest]?.mod ?? -Infinity)
                    ? ability
                    : largest,
    ...
    get availableAbilities() {
        ...
        if (this.item.system.availableAbilities)
        ...
        if (this.attack.type.classification === 'spell')
            return new Set(
                this.actor?.system.attributes?.spellcasting
                    ? [this.actor.system.attributes.spellcasting]
                    : Object.values(this.actor?.spellcastingClasses ?? {}).map(
                          (c) => c.spellcasting.ability
                      )
            )
    ...
    static transformTypeData(source, activityData, options) {
        ...
        return foundry.utils.mergeObject(activityData, {
            attack: {
                ...
                type: {
                    value: source.system.actionType.startsWith('m')
                        ? 'melee'
                        : 'ranged',
                    classification: source.system.actionType.endsWith('wak')
                        ? 'weapon'
                        : 'spell',
                },
            },
            damage: {
                ...
                parts:
                    damageParts.map((part) =>
                        this.transformDamagePartData(source, part)
                    ) ?? [],
            },
        })
    }
    ...
    prepareData() {
        ...
        this.attack.type.classification ||=
            this.item.system.attackClassification ?? ''
    }
    ...
    prepareFinalData(rollData) {
        if (
            this.damage.includeBase &&
            this.item.system.offersBaseDamage &&
            this.item.system.damage.base.formula
        ) {
            const basePart = this.item.system.damage.base.clone(
                this.item.system.damage.base.toObject(false)
            )
            basePart.base = true
        ...
        this.labels.modifier =
            simplifyRollFormula(roll.formula, { deterministic: true }) || '0'
    ...
    getAttackData() {
        ...
        if (item.magicalBonus && item.magicAvailable && !this.attack.flat)
            parts.push(item.magicalBonus)
    ...
    getDamageConfig(config = {}) {
        ...
        if (ammo) {
            const properties = Array.from(ammo.properties).filter(
                (p) => CONFIG.DND5E.itemProperties[p]?.isPhysical
            )
            if (
                this.item.system.properties?.has('mgc') &&
            ...
            for (const roll of rollConfig.rolls) {
                for (const property of properties) {
                    if (!roll.options.properties.includes(property))
                        roll.options.properties.push(property)
                }
            }
    ...
    _processDamagePart(damage, rollConfig, rollData, index = 0) {
        if (!damage.base)
            return super._processDamagePart(damage, rollConfig, rollData, index)

        // Swap base damage for versatile if two-handed attack is made on versatile weapon
        if (
            this.item.system.isVersatile &&
            rollConfig.attackMode === 'twoHanded'
        ) {
        ...
        const roll = super._processDamagePart(
            damage,
            rollConfig,
            rollData,
            index
        ...
        if (this.item.type === 'weapon') {
            ...
            const includeMod =
                rollConfig.attackMode !== 'offhand' || roll.data.mod < 0
            if (includeMod && !roll.parts.some((p) => p.includes('@mod')))
                roll.parts.push('@mod')

            // Add magical bonus
            if (
                this.item.system.magicalBonus &&
                this.item.system.magicAvailable
            ) {
